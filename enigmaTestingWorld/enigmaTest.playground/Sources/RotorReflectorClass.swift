...
open class Rotor {
    ...
    public init(rotorId:Int , encodingNo:Int, rotorPos:Int) {
        ...
        if let encoding = self.rotorEncodingStr {
            ...
            print("WARNING: Rotor encoding is nil.")
        }
    }
     ...
     */
    public func bkwdEncrypt (char:Character) -> Character {
        let tmpIndx:Int = charToIntWrapper(char: char, defaultReturnInt: 0)
        let tmpChar:Character = self.invRotorEncodingArr[tmpIndx]
        var newCharPos:Int = charToIntWrapper(char: tmpChar, fn: {(pos:Int)->Int in return (pos - self.rotorPos) % 26}, defaultReturnInt: 0)
        if newCharPos < 0 {
            newCharPos = 26 + newCharPos
        }
        return Character(UnicodeScalar(newCharPos + 65)!)
    }
    ...
    public func setUpRotor() throws {
        ...
        if !selectedEncodingNos.contains(self.rotorEncodingNo) {
            ...
            throw RotorError.invalidSelection(message: "WARNING: Rotor encoding \(self.rotorEncodingNo) is already in use.")
        }
    ...
    public func bkwdRotorOp (inputChar:Character) -> Character {
        ...
        let outputChar = bkwdEncrypt(char: inputChar)
        print("BKWD ROTOR OP: ")
        print("inputChar:           \(inputChar)")
        print("outputChar:          \(outputChar)")
        print()
...
public enum ReflectorError:Error {
    case invalidSelection(message:String)
    case invalidCharacter(message:String)
}
...
public class Reflector {
    ...
    public let reflectorId:Int
    public let reflectorEncodingNo:Int
    public let reflectorEncodingStr:String?
    public var reflectorEncodingArr:[Character] = []
    public var isValidReflector: Bool = false
    ...
    public init(reflectorId:Int, encodingNo:Int) {
        ...
        self.reflectorEncodingNo = encodingNo
        
        self.reflectorEncodingStr = ReflectorEncodingsDict[encodingNo]
        if let encoding = self.reflectorEncodingStr {
            self.reflectorEncodingArr = Array(encoding)
        } else {
            print("WARNING: Reflector encoding is nil.")
        }
    }
    
    
    public func encrypt(char: Character) -> Character {
        let newIndx: Int = charToIntWrapper(char: char, defaultReturnInt: 0)
        return self.reflectorEncodingArr[newIndx]
    }
    
    public func checkIfNil() throws {
        guard self.reflectorEncodingStr != nil else {
            throw ReflectorError.invalidSelection(message: "ERROR: Invalid encoding number selected.")
        }
    }
    /**
    ...
    public func reflectorOp(char:Character) -> Character {
        ...
        let outChar = encrypt(char: char)
        print("REFLECTOR OP: ")
        print("inputChar:           \(char)")
        print("outputChar:          \(outChar)")
        print()
        return outChar
    }
